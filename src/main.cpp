...
double mph2mps(double mph) { return (mph / 3600) * 1609.34; }
...
int main() {
  ...
  h.onMessage([&target_velocity,&lane,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i = 0; i < sensor_fusion.size(); i++) {
				...
				// estimate position when at end of waypoints assuming
				// constant speed
